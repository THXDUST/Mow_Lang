        ...
        if token.type == "EOF" and DEPTH == 0:
            ...
            if token.aux in (keywords[0], keywords[1]):  # PRINT
                ...
                if next_token.type == "left_parenthesis":
                    ...
                    result = str(MI.EvalExpr(expr_tokens, GLOBALS, LINES)) if expr_tokens else ""
                ...
                # Coletar tokens do corpo da classe
                class_body_tokens = []
                depth = 1  # Já entramos no primeiro {
                ...
                # Processar o corpo da classe
                methods = {}
                properties = {}
                
                # Parser simples para métodos e propriedades
                j = 0
                while j < len(class_body_tokens):
                    ...
                    if member_token.type == "keyword" and member_token.aux == "FUNCTION":
                        ...
                        continue  # Ignorar ponto e vírgula extra
                    else:
                        #MI.ThrowSyntaxError(f"Unexpected token in class body: {member_token.type}", member_token, GLOBALS)
                        j = MI.ClearWhitespace(class_body_tokens, j)  # Apenas ignorar tokens inesperados por enquanto
                        
                # Criar e registrar a classe
                ...
                i = MI.ClearWhitespace(Tokens, i)
                if i >= len(Tokens):
                    MI.ThrowSyntaxError("Expected '('", token, GLOBALS)
            ...
            if next_token.type == "left_parenthesis":
            ...
            elif token.aux in GLOBALS["CLASSES"]:  # Se o identificador atual é uma classe
                # Criar instância de classe
                ...
                # Acesso a método ou propriedade via ponto
                if next_token.type == "dot":
                    ...
                    # Verificar se é chamada de método
                    if a.type == "left_parenthesis":
                        # Chamada de método
                        if hasattr(var.value, 'class_def') and member_name in var.value.class_def.methods:
                            ...
                            # Executar método (simplificado - em um ambiente real seria mais complexo)
                            ...
                            try:
                                # Adicionar 'this' ou 'self' como primeiro parâmetro
                    ...
                    # Acesso a propriedade
                        ...
                        if hasattr(var.value, 'attributes') and member_name in var.value.attributes:
                            ...
                            while i < len(Tokens):
                                ...
                                if next_token2.type == "semicolon":
                                    ...
                                    # Obter o tipo esperado da propriedade
                                    prop_info = var.value.class_def.properties.get(member_name)
                                    if prop_info:
                                        prop_type, _ = prop_info
                                        # Converter o valor para o tipo correto da propriedade
                        ...
                        # Apenas acesso a propriedade (leitura)
                        if hasattr(var.value, 'attributes') and member_name in var.value.attributes:
...
def Main():
    ...
    FILE = OpenFile(r"ErrorsTesting2.mow")
