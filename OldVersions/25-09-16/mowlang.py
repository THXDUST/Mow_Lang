...
class Var:
    ...
    def __init__(self, ref_name:str, typeof:str, value:any, universal:bool=False):
        typeof = typeof.lower()
        ft = typeof
...
class Func:
    """
        CREATES FUNCTIONS
    """
    def __init__(self, name:str, refline:int, args:list, file:str, filealias:str=None):
        self.args={}
        self.name = name
        self.line = refline
        self.file = file
        self.filealias = filealias if filealias else file
...
def __DEBUG_INFO__(level="WARN", write_to_file=False, close_program=False, message="Verify language developer, no argument provided to logging this error", line=-1, exit_code=-1, locals=__GLOBALS__):
    ...
    if level == "INFO":
        ...
        writable = f"TRACING RECENT CALL:\n\t[{level}] {message}\n\tKEYWORD: {__OPEN_FILE__(currentFile, aliases[currentFile] if currentFile in aliases else False)[0][line].strip()}\n\tAT LINE: {line}\n\tFILE: {currentFile if currentFile == 'local' else currentFile+'.mow'}"
        try:
            writable = f"TRACING RECENT CALL:\n\t[{level}] {message}\n\tKEYWORD: {locals['__FILE_LINES__'][aliases[currentFile] if currentFile in aliases else currentFile][line-1].strip()}\n\tAT LINE: {line}\n\tFILE: {currentFile if currentFile == 'local' else (aliases[currentFile] if currentFile in aliases else currentFile)+'.mow'}\n" if not __CONSOLE_MODE__ else f"TRACING RECENT CALL:\n\t[{level}] {message}\n\tKEYWORD: {locals['__FILE_LINES__']['local'][-1]}\n\tAT LINE: 1\n"
        except KeyError:
...
def __CONVERT_TO__(value, target_type, cline=-1, error:bool=True, locals:dict={}):
    ...
    if target_type == "str":
        ...
        lit2, s = __RUN__(value, cline, locals, False)
        if s and type(lit2) in [int, float]:
            return [int(lit2), True]
...
def __OPEN_FILE__(filepath, alias=False):
    """
        PRETTY MUCH SELF-EXPLANATORY, BUT IT JUST OPENS FILES AND STORES THEM INSIDE \_\_GLOBALS\_\_
    """
    tmpScope = aliases[alias] if alias and alias in aliases else (aliases[filepath] if filepath in aliases else filepath)
    tmpFilepath = aliases[alias] if alias and alias in aliases else filepath

    #print(522, tmpScope, tmpFilepath, filepath, alias, aliases)

    try:
        return [__GLOBALS__["__FILE_LINES__"][alias], True]
    except KeyError:
        try:
            return [__GLOBALS__["__FILE_LINES__"][tmpScope], True]
        except KeyError:
            pass
        ...
        try:
            return [__GLOBALS__["__FILE_LINES__"][tmpFilepath], True]
        except KeyError:
            pass
        ...
        try:
            with open(tmpFilepath, "r") as f:
                data = f.readlines()
            __GLOBALS__["__FILE_LINES__"][tmpScope] = data
            return [data, True]
        except FileNotFoundError:
            pass

        try:
            with open(tmpFilepath+".mow", "r") as f:
                data = f.readlines()
            __GLOBALS__["__FILE_LINES__"][tmpScope] = data
            return [data, True]
        except FileNotFoundError:
        ...
        try:
            with open(filepath, "r") as f:
                data = f.readlines()
            __GLOBALS__["__FILE_LINES__"][alias] = data
            return [data, True]
        except FileNotFoundError:
            pass
...
def __PARSE_FUNC__(maybefunc: str, warn: bool = True, cline: int = -1, locals:dict={}, scope:str="local"):
    ...
    scope = aliases[currentFile] if currentFile in aliases and scope == "cfile" else scope

    s = maybefunc.strip()

    #print(582, maybefunc, warn, cline, scope)
    ...
    while i < len(s):
        c = s[i]
        #print(610, inner_chars, i, depth_paren, in_str, esc, pos, s)
        if in_str:
    ...
    for i, arg in enumerate(args):
        arg_val, succ = __CONVERT_TO__(arg, "*", cline, False, locals)
        args[i] = arg_val if succ else arg
...
def __CREATE_LOCALS__(predef:dict|bool=False) -> dict:
    ...
    if predef:
        locals = {
            ...
            "__STARTING_POINT__": predef["__STARTING_POINT__"]
        }
    else:
        locals = {
            "__NAME__": __name__,
            ...
            "__OS_NAME__": os.name,
            "__PLATFORM__": sys.platform,
            "__STARTING_POINT__": aliases[currentFile]+".mow" if currentFile in aliases else currentFile
        }
...
def printdict(callingline:int, d:dict, indent=0):
    """
        PRINTS DICTS NICELY
    """
...
def split_expression(expr):
    """
    Separa a expressão em números, variáveis e chamadas de função.
    Exemplo: (1+a)/2*functionTeste() -> ['(', 1, '+', 'a', ')', '/', 2, '*', 'functionTeste()']
    """
    # Regex: números, palavras (variáveis/funções), operadores, parênteses
    token_pattern = r'(\b\w+\s*\([^)]*\)|\d+\.\d+|\d+|[a-zA-Z_]\w*|[+\-*/%()])'
    tokens = re.findall(token_pattern, expr)
    # Limpa espaços e converte números
    result = []
    for t in tokens:
        t = t.strip()
        if not t:
...
# Exemplo de uso:
# split_expression("(1+a)/2*functionTeste()")
# Saída: ['(', 1, '+', 'a', ')', '/', 2, '*', 'functionTeste()']

def eval_expression(expr, cline, locals):
    """
    Executa cada item da expressão usando __RUN__ e monta a expressão final para eval.
    """
    ...
    for t in tokens:
        if isinstance(t, (int, float)):
            ...
            # Variável ou função: executa no interpretador
            val, succ = __RUN__(str(t), cline, locals, False)
            # Se for string, coloca aspas
            if isinstance(val, str):
                resolved.append(f'"{val}"')
            else:
                resolved.append(str(val))
    expr_final = "".join(resolved)
    #print(f"Evaluating expression: {expr_final}")
    return eval(expr_final)

def is_arithmetic_expression(expr: str, cline: int, locals: dict):
    pattern = r'([a-zA-Z_]\w*|\d+(\.\d+)?|\([^)]*\))\s*[\+\-\*/%]\s*([a-zA-Z_]\w*|\d+(\.\d+)?|\([^)]*\))'
    is_expr = re.search(pattern, expr) is not None
    if is_expr:
        expr = eval_expression(expr, cline, locals)
    return [expr, is_expr]
...
def __RUN__(__LINE__:str, cline:int, locals:dict, warn: bool = True, *kargs):
    ...
    with open("console.log", "a") as f:
        f.write(f"\nRunning {__LINE__.strip()} in {currentFile}, line: {cline}")
    try:
        __LINE__ = __LINE__.split("-->")[0].strip()
    except AttributeError:
        if warn:
            return __DEBUG_INFO__(level="ERROR", write_to_file=True, close_program=True, message="INVALID SYNTAX, MAKE SURE TO PROVIDE A VALID COMMAND", line=cline)
    ...
    try:
        funcargs = __LINE__.split("(")[1:]
    except IndexError:
        funcargs = []

    args = __LINE__.split()[1:]

    tmpFile = aliases[currentFile] if currentFile in aliases else currentFile
    tmpScope = currentFile
    #print("\n",833, f"USING SCOPE: {tmpScope} FOR FILE {tmpFile}", "\n")
    ...
    # SEQUENCE OF COMPARATION OPERATORS
        ...
        try:
            ...
            comp2, t2 =__CONVERT_TO__(cmp, "*", cline, False, locals)

            if not t:
                comp, t = __RUN__(top, cline, locals, False)

            if not t2:
                comp2, t2 = __RUN__(cmp, cline, locals, False)
        ...
        try:
            ...
            return __DEBUG_INFO__("ERROR", True, True, "CANNOT COMPARE NON-NUMERIC VALUES", cline, -1, locals)
    ...
    # SEQUENCE OF ARITHMETIC OPERATORS
        ...
        arg2, s = __RUN__(arg2, cline, locals, False)
        if not s:
            literal, s = __IS_STRING__(arg2)
            if s:
                arg2 = arg2[1:-1]
            else:
                arg2, s = __CONVERT_TO__(arg2, "float", cline, False, locals=locals)
        ...
        try:
            arg1, arg2 = args[0].split(",")[0].strip(), args[1].split(",")[0].strip()
        except IndexError:
        ...
        if t1 not in [int, float] or t2 not in [int, float]:
            return __DEBUG_INFO__("ERROR", True, True, f"CANNOT SUBTRACT {str(t1)} FROM {str(t2)}", cline, -1, locals)
        ...
        arg1 = float(arg1) if float(arg1) == arg1 else arg1
        arg2 = float(arg2) if float(arg2) == arg2 else arg2
        ...
        #print(1418, arg1, arg2, t1, t2)

        if t1 not in [int, float] or t2 not in [int, float]:
            return __DEBUG_INFO__(level="ERROR", write_to_file=True, close_program=True, message=f"CANNOT DIVIDE {str(t2)} BY {str(t1)}", line=cline)
    ...
    # VARIABLE OPERATORS
        ...
        if typeof != "null":
            literal, is_string = __IS_STRING__(value)
            result, succ = __RUN__(value, cline, locals, False)
            if not succ:
                result, succ = __CONVERT_TO__(value, typeof, cline, False, locals)

            if typeof in ["str", "char"]:
                if is_string and typeof == "str":
                    result = literal[1:-1]
                elif is_string and typeof == "char":
                    result = str(result)[0]
            if not succ:
                return [result, succ]
        ...
        tmpS = aliases[tmpScope]
        
        if not tmpS in locals["__VARIABLES__"]:
            locals["__VARIABLES__"][tmpS] = {}

        locals["__VARIABLES__"][tmpS][name] = var
        ...
        try:
            val = locals["__VARIABLES__"][tmpScope][namearg].value
        except KeyError:
            try:
                val = __GLOBALS__["__VARIABLES__"][tmpScope][namearg].value
            except KeyError:
                if '[' in namearg and namearg.split("[")[0][-1] != ' ':
                    return __SEARCH_DICT_LIST__(namearg, locals, tmpScope, cline)
    ...
    # LIBRARY MANAGER
        ...
        try:
            alias = args[2] if args[1] == "AS" else False
        except IndexError:
            alias = "".join(file.split(".mow")[:-1])
        
        locals2 = __CREATE_LOCALS__()
        locals2["__VARIABLES__"] = {alias: {}}
        locals2["__FUNCTIONS__"] = {}
        aliases[alias] = file
        aliases[file] = alias
        
        importlines, s = __OPEN_FILE__(file, alias)
        currentFile = alias
        if not s:
            file2 = file+".mow"
            importlines, s = __OPEN_FILE__(file2, alias)
            currentFile = alias
            recentOpenFiles.append(currentFile)
            if not s:
    ...
    # INTERPRETER BEHAVIOUR CONFIGURATOR
        ...
        if method == "NO_WARNS":
            ...
            with open(__GLOBALS__["__PROGRAM_FILEPATH__"]+":version", "w") as vf:
                vf.write(str(ver2))
    ...
    # FUNCTION CREATOR
        ...
        name = args[0]
        params = args[1:]
        func = Func(name, cline, params, tmpFile, tmpScope)
        recentOpenFuncs.append(name)

        if not (kargs[0] if len(kargs) > 0 else False):
            if tmpScope not in locals["__FUNCTIONS__"]:
                locals["__FUNCTIONS__"][tmpScope] = {}
            locals["__FUNCTIONS__"][tmpScope][name] = func
    ...
    # EXPORT SPECIFIC FUNCTIONS/VARIABLES FROM MODULE
        ...
        if tyype == "func":
            for f in toExpt:
                try:
                    if tmpScope not in __GLOBALS__["__FUNCTIONS__"]:
                        __GLOBALS__["__FUNCTIONS__"][tmpScope] = {}
                    __GLOBALS__["__FUNCTIONS__"][tmpScope][f] = locals["__FUNCTIONS__"][tmpScope][f]
                except KeyError:
                    return __DEBUG_INFO__("ERROR", True, True,
                        f"FUNCTION {f} NOT FOUND IN FILE {tmpFile}.mow TO EXPORT", cline)
        elif tyype == "var":
            for v in toExpt:
                try:
                    if tmpScope not in __GLOBALS__["__VARIABLES__"]:
                        __GLOBALS__["__VARIABLES__"][tmpScope] = {}
                    __GLOBALS__["__VARIABLES__"][tmpScope][v] = locals["__VARIABLES__"][tmpScope][v]
                except KeyError:
                    return __DEBUG_INFO__("ERROR", True, True,
                        f"VARIABLE {v} NOT FOUND IN FILE {tmpFile}.mow TO EXPORT", cline)
        else:
            return __DEBUG_INFO__("ERROR", True, True,
    ...
    # RAISE ERROR IF __RUN__ NOT CALLED INTERNALLY BY INTERPRETER
        ...
        expr, is_expr = is_arithmetic_expression(__LINE__, cline, locals)
        if is_expr:
            parts = __LINE__.split()
            if len(parts) > 1:
                expr_only = " ".join(parts[1:])
                return [expr_only, True]
            else:
                return [expr, True]
        else:
            try:
                expr, is_expr = is_arithmetic_expression("".join(__LINE__.split(c)[1:]).strip(), cline, locals)
                if is_expr:
                    return [expr, True]
            except ValueError:
                pass

        #func, argss = __PARSE_FUNC__(method+" ".join(args) if method else c if len(args) == 0 else c+"."+method+" ".join(args), False, cline, locals, c.strip() if method else "local")
        pbFunc = ""
        tmpScope2 = "cfile"
        #print(2017, method, args, c, funcargs)

        #print(2012, pbFunc, method, args, c)

        #print(1940, aliases, c)

        try:
            pbFunc = __LINE__.strip().split(c.strip())[1].strip().split(".", 1)[1]
            tmpScope2 = c.strip()
        except IndexError:
            pbFunc = __LINE__.strip()
            tmpScope2 = "cfile"

        print(f"'{pbFunc}'", f"'{tmpScope2}'")

        func, argss = __PARSE_FUNC__(pbFunc, False, cline, locals, tmpScope2)
        if not argss:
            func, args2 = __PARSE_FUNC__(pbFunc, False, cline, __GLOBALS__, tmpScope2)

        args2 = ''
        #print(func, argss, args2)
        ...
        if argss:
            ...
            val, s = __RUN_FUNC__(fargs, func, locals, func.file)
            recentOpenFiles.pop()
            currentFile = recentOpenFiles[-1]
            return [val, s]
        elif args2:
        ...
        try:
            return [locals["__VARIABLES__"][tmpScope][c].value, True]
        except KeyError:
            try:
                return [locals["__VARIABLES__"][currentFile][c].value, True]
            except KeyError:
                try:
                    return [locals["__VARIABLES__"][aliases[currentFile]][c].value, True]
                except KeyError:
                    try:
                        return [locals["__VARIABLES__"][aliases[tmpScope]][c].value, True]
                    except KeyError:
                        try:
                            return [locals["__VARIABLES__"][tmpScope2][c].value, True]
                        except KeyError:
                            pass

        if warn:
            return __DEBUG_INFO__("ERROR", True, True, f"KEYWORD {c} IS NOT DEFINED", line=cline, locals=locals)
        return [__LINE__, False]
...
def __PRE_RUN__(command:str, lines:list, locals:dict, optline:int=0, topre:list=[]):
    ...
    """if __GLOBALS__["__FILE_LINES__"]["local"] != locals["__FILE_LINES__"]["local"]:
        #print("\n-----------------")
        printdict(2107, locals)
        #print("-----------------\n")"""
...
def __RUN_FUNC__(args:dict, func:Func, locals:dict, scope:str="local"):
    ...
    funcArgs = func.args
    file = func.file
    lines:list = __OPEN_FILE__(file)[0]
    recentOpenFiles.append(file)
    currentFile = file
    funcReturn:any = None
    cline:int = func.line
    ...
    for name, val in args.items():
        ...
        #print(__LOCALS__["__VARIABLES__"],"\n\r", locals["__VARIABLES__"])
        if scope not in __LOCALS__["__VARIABLES__"]:
            __LOCALS__["__VARIABLES__"][scope] = {}
        __LOCALS__["__VARIABLES__"][scope][funcArgs[name]] = Var(funcArgs[name], ts, val)
        #print(f"[DEBUG] ARG {name} ({funcArgs[name]}) = {val} ({ts})")
...
def interpret():
    ...
    """
    global __GLOBALS__, insideFunc, shouldRunFuncCode, __TO_PRERUN__, currentFile, aliases
    for __FILE__ in __GLOBALS__["__FILES__"]:
        __GLOBALS__["__STARTING_POINT__"] = __FILE__
        __GLOBALS__["__PROGRAM_FILEPATH__"] = __FILE__
        __GLOBALS__["__FILE_LINES__"] = {}
        __GLOBALS__["__FILE_LINES__"]["local"], succ = __OPEN_FILE__(__FILE__, "local")
...
def console():
    ...
    print(f"MowLang v{__CURRENT_VERSION__} (BUILD {__BUILD__}) ({now}) [platform: {plat} || sys: {platform} || type: {__GLOBALS__['__OS_NAME__']}]")
    if __GLOBALS__["__DEBUG__"]:
        print("Debug mode: ON")
    ...
    try:
        keyword = input("~> ")
        status = ""
        while keyword.split()[0].split(".")[0] != "EXIT":
